      ...
      <main className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            {user.role === "admin" && <Link className="admin-link" href="/admin">Супер админ</Link>}
            <div className="user-chip"><span>{user.name.charAt(0).toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.role === "admin" ? "Супер админ" : user.role} · {user.email}</small></div></div>
          </div>
        </header>
